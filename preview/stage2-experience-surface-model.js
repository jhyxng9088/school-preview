...
export function buildExperienceSurfaceModel(state = {}) {
  ...
  if (primary === 'class-active') {
    ...
    detail = timeRange(school.current) || '수업 진행 중'
    support = school.next ? `다음 ${periodLabel(school.next)} · ${subjectOf(school.next)}` : '오늘 마지막 수업'
  } else if (primary === 'break-time') {
    tone = 'break'
    eyebrow = '쉬는 시간'
    title = school.next ? `다음 · ${subjectOf(school.next)}` : '잠깐 쉬어가자'
    detail = school.next ? `${periodLabel(school.next)} · ${school.next.start || ''} 시작` : '오늘 수업이 모두 끝났어.'
    support = school.next ? '다음 수업을 준비할 시간' : ''
  } else if (primary === 'lunch-time') {
    tone = 'lunch'
    eyebrow = '점심시간'
    title = '점심 먹을 시간'
    detail = school.next ? `다음 ${periodLabel(school.next)} · ${subjectOf(school.next)}` : '오후 일정 확인하기'
    support = '학교생활의 흐름을 잠깐 느슨하게'
  } else if (primary === 'before-school') {
    tone = 'before'
    eyebrow = '등교 전'
    title = school.next ? subjectOf(school.next) : '오늘 준비'
    detail = school.next ? `${periodLabel(school.next)} · ${school.next.start || ''} 시작` : '첫 수업을 확인해줘.'
    support = '하루가 시작되기 전'
  } else if (primary === 'after-school') {
    tone = 'after'
    eyebrow = '수업 종료'
    title = '오늘 수업 끝'
    detail = school.last ? `${periodLabel(school.last)} · ${subjectOf(school.last)}까지 완료` : '오늘 정규 수업이 끝났어.'
    support = '남은 리마인더나 Study를 확인해봐.'
  } else if (primary === 'study-active') {
    tone = 'study'
    eyebrow = study.paused ? 'Study · 일시정지' : 'Study · 진행 중'
    title = String(study.subject || '').trim() || '집중 중'
    detail = study.paused ? '다시 시작하면 이어서 기록돼.' : '지금의 집중 흐름을 유지하고 있어.'
